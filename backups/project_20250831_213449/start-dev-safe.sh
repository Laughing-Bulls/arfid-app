...
echo -e "${BLUE}🚀 Starting React Native Development Environment (Safe Mode)${NC}"
echo -e "${YELLOW}⚠️  This script preserves AsyncStorage data by NOT using -c flag${NC}"
...
# Start Metro bundler in background WITHOUT cache clearing
echo -e "${BLUE}📦 Starting Metro bundler (preserving cache and AsyncStorage)...${NC}"
...
if wait_for_metro; then
    ...
    echo -e "${GREEN}💾 AsyncStorage data is preserved${NC}"
    echo -e "${YELLOW}💡 Press Ctrl+C to stop both processes${NC}"
    echo -e "${YELLOW}💡 Use start-dev.sh if you need to clear cache${NC}"
