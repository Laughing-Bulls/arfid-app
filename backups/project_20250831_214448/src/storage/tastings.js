 ...
 * @param {string|Object|null} input - Photo input (legacy string, photo object, or null)
 * @returns {Object|null} Normalized photo object or null
 */
export function normalizePhoto(input) {
  if (!input) return null;
  if (typeof input === 'string') return { kind: 'uri', uri: input }; // legacy
  if (input.kind === 'stock') return { kind: 'stock', id: input.id };
  return { kind: 'uri', uri: input.uri || '' };
}

/**
 * Convert photo object to storage format (for database compatibility)
 * @param {Object|null} photo - Normalized photo object
 * @returns {string|null} Photo URI string for storage
 */
function photoToStorage(photo) {
  if (!photo) return null;
  if (photo.kind === 'stock') return `stock:${photo.id}`;
  return photo.uri;
}

/**
 * Convert storage format to photo object
 * @param {string|null} photoUri - Photo URI from storage
 * @returns {Object|null} Normalized photo object
 */
function photoFromStorage(photoUri) {
  if (!photoUri) return null;
  if (photoUri.startsWith('stock:')) {
    return { kind: 'stock', id: photoUri.substring(6) };
  }
  return { kind: 'uri', uri: photoUri };
}
 ...
 * @param {string|Object} item.photo - The photo (URI string or photo object)
...
export const addItem = async (item) => {
  try {
    ...
    // Normalize photo and convert to storage format
    const normalizedPhoto = normalizePhoto(item.photo);
    const photoUri = photoToStorage(normalizedPhoto);
...
export const getAllItems = async () => {
  try {
    ...
    // Normalize photos and remove legacy photoUri
    return items.map(item => {
      const { photoUri, ...rest } = item;
      return {
        ...rest,
        photo: photoFromStorage(photoUri)
      };
    });
  } catch (error) {
...
export const getItemsByCategory = async (category) => {
  try {
    const items = await ItemOperations.getItemsByCategory(category);
...
export const searchItems = async (searchTerm) => {
  try {
    const items = await ItemOperations.searchItemsByTitle(searchTerm);
  ...
  try {
    ...
    if (item) {
      ...
      return {
        ...rest,
        photo: photoFromStorage(photoUri),
        tries: events.map(event => event.triedAt).sort()
      };
    }
...
export const updateItem = async (id, updates) => {
  try {
    // If photo is being updated, normalize it and convert to storage format
    if (updates.photo !== undefined) {
      const normalizedPhoto = normalizePhoto(updates.photo);
      updates.photoUri = photoToStorage(normalizedPhoto);
      delete updates.photo; // Remove photo from updates since we're storing photoUri
    }
