...
export default function NewTastingScreen({ navigation, route }) {
  ...
  // Handle photo param from StockPicker
  useEffect(() => {
    const picked = routeHook.params?.photo;
    if (picked) {
      setPhoto(picked);                          // your local state setter
      navigationHook.setParams({ photo: undefined }); // clear after consuming
    }
  }, [routeHook.params?.photo]);
